...
def write_section(md, section_data):
    ...
    for data_source in section_data:
        ...
        if new_state and current_state != 'Start':
            md.write('\t</details>\n')
        if new_state:
            write_state_header(md, data_source)
    ...
    md.write('\t<details>\n')
    md.write(f'\t\t<summary><font size="+1">National and multistate</font></summary>\n')
...
def write_state_header(md, data_source):
    md.write('\t<details>\n')
    md.write(f'\t\t<summary><font size="+1">{data_source["state"]}</font></summary>\n')


def write_scraper(md, data_source):
    md.write('\t\t<details>\n')
    md.write(f'\t\t\t<summary><u>{data_source["name"]}</u></summary>\n')
    md.write(f'\t\t\t{"&emsp;" * 2}<b>Agency described:</b> {data_source["agency_described"]}<br>\n')
