...
def save_file(file):
	...
	folder_name = folder + file.replace('.pdf','/') # Converts the file's name into a folder
	...
	for table in enumerate(tqdm(tables, desc="Tables",position=1,leave=True), start=1):
		try:
			#print("Working... this may take a while")
			tabula.io.convert_into(tables,folder_name,output_format='csv',pages="all")
